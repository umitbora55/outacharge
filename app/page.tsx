  ...
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-slate-800 rounded-2xl w-full max-w-md p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setShowForgotPassword(false); setForgotEmail(""); setForgotSent(false); }} className="text-slate-400 hover:text-white">
          <X className="w-6 h-6" />
        </button>
      </div>
      ...
      {forgotSent ? (
        <div className="text-center">
          ...
          <h4 className="text-white font-medium mb-2">E-posta Gönderildi!</h4>
          <p className="text-slate-400 text-sm mb-4">
            Şifre sıfırlama linki <strong>{forgotEmail}</strong> adresine gönderildi. Lütfen e-postanızı kontrol edin.
          </p>
          <p className="text-slate-500 text-xs mb-4">Link 1 saat geçerlidir.</p>
          <button
            onClick={() => { setShowForgotPassword(false); setShowLoginModal(true); setForgotSent(false); setForgotEmail(""); }}
            className="w-full py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-full font-medium transition"
          >
            Giriş Ekranına Dön
          </button>
        </div>
        ...
        <div className="space-y-4">
          <p className="text-slate-400 text-sm">E-posta adresinizi girin, şifre sıfırlama linki gönderelim.</p>
          <div>
            <label className="block text-slate-300 text-sm font-medium mb-2">E-posta</label>
            <input 
              type="email" 
          ...
          <button 
            onClick={async () => {
              if (!forgotEmail.trim()) {
                setToast({ show: true, message: "E-posta adresi girin.", type: "error" });
                setTimeout(() => setToast({ show: false, message: "", type: "success" }), 3000);
                return;
              }
            ...
            className="w-full py-3 bg-emerald-500 hover:bg-emerald-600 disabled:bg-emerald-500/50 text-white rounded-full font-medium transition flex items-center justify-center gap-2"
          >
            {forgotLoading ? <><Loader2 className="w-5 h-5 animate-spin" />Gönderiliyor...</> : "Sıfırlama Linki Gönder"}
          </button>
          <p className="text-center text-slate-400 text-sm">
            <button onClick={() => { setShowForgotPassword(false); setShowLoginModal(true); }} className="text-emerald-400 hover:text-emerald-300">
              Giriş ekranına dön
            </button>
          </p>
        </div>
